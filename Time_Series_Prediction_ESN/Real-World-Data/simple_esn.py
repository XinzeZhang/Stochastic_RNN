...
class SimpleESN(BaseEstimator, TransformerMixin):
    ...
    components_ : array_like, shape (lens_ts, 1+n_features+n_components)
    ...
    >>> import numpy as np
    >>> lens_ts, n_features = 10, 5
    >>> np.random.seed(0)
    >>> X = np.random.randn(lens_ts, n_features)
    ...
    def _fit_transform(self, X):
        lens_ts, n_features = X.shape
        # lens_ts means the lens of the time sequence
        # n_features means the features in every input time
        ...
        esn_parameters={
            'esn_input_shape': X.shape,
            'esn_input_ts_lens': lens_ts,
            'esn_ts_features': n_features,
            'esn_input_weights_shape': self.input_weights_.shape,
            'esn_hidden_units':self.n_components,
            'esn_hidden_units_out':self.n_readout,
            'esn_hidden_weights_shape':self.weights_.shape
        }
        pprint(esn_parameters)

        curr_ = zeros(shape=(self.n_components, 1))
        U = concatenate((ones(shape=(lens_ts, 1)), X), axis=1)
        for t in range(lens_ts):
    ...
    def fit(self, X, y=None):
        ...
        Parameters
        ----------
        X : array-like shape, (lens_ts, n_features)
    ...
    def fit_transform(self, X, y=None):
        ...
        Parameters
        ----------
        X : array-like of shape [lens_ts, n_features]
            training set.

        Returns
        -------
        readout : array, shape (lens_ts, n_readout)
